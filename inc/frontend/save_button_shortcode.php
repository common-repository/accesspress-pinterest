...
// pin button shape
$button_shape = isset($attr['shape']) ? esc_attr($attr['shape']): 'rectangular'; //round or rectangular
// small or large
$button_size = isset($attr['size']) ? esc_attr($attr['size']) : 'large'; // small or large
$data_pin_save='true';

$post_title = $post->post_title;
$post_url   = get_permalink(); //$this->curPageURL();

$save_pin_type = isset($attr['type']) ? esc_attr($attr['type']) : 'any-image'; //one-image or any-image
if ($save_pin_type == "one-image"){
	$url = isset($attr['save_url']) ? esc_url($attr['save_url']) : $post_url;
	$pin_image =  isset($attr['save_image']) ? esc_attr($attr['save_image']) : '';
	$image_description = isset($attr['image_description']) ? esc_attr($attr['image_description']) : $post_title;
	
	$data_pin_count= isset($attr['count_position']) ? esc_attr($attr['count_position']) : '';
